...
const itemImages = document.querySelectorAll(".item-img img");
...
function updatePreviewImage(index) {
  if (currentImageIndex !== index) {
    ...
    gsap.to(previewImage, {
      ...
      ease: "power2.out",
      onComplete: () => {
        // Change l'image quand le fade-out est fini
...
function animate() {
  ...
  if (currentTranslate < lastTranslate && window.innerWidth > 940) {
    // On scroll vers la droite/bas : cache le menu
    document.querySelector(".menu").style.transform = "translateY(-100%)";
  } else {
    // On scroll vers la gauche/haut : montre le menu
